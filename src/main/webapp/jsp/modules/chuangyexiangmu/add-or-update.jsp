    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑创业项目</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item">创业项目管理</li>
                        <li class="breadcrumb-item active">编辑创业项目</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">创业项目信息</h3>
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="jiaoshiId" name="jiaoshiId" type="hidden">
                                   <div class="form-group col-md-6">
                                       ...
                                       <input name="file" type="file" id="chuangyexiangmuPhotoupload"
                                              class="form-control-file">
                                       <input name="chuangyexiangmuPhoto" id="chuangyexiangmuPhoto-input" type="hidden">
                                   </div>
                                   <div class="form-group col-md-6">
                                       <label>项目分类名称</label>
                                   ...
                                   <div class="form-group  col-md-6">
                                       <label>项目具体内容</label>
                                       <input id="chuangyexiangmuContentupload" name="file" type="file">
                                       <script id="chuangyexiangmuContentEditor" type="text/plain"
                                               style="width:100%;height:230px;"></script>
                                       <script type = "text/javascript" >
                                       //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
                                       //相见文档配置属于你自己的编译器
                                       var chuangyexiangmuContentUe = UE.getEditor('chuangyexiangmuContentEditor', {
                                           toolbars: [
                                               [
                                                   'undo', //撤销
                                                   'bold', //加粗
                                       ...
                                       <input type="hidden" name="chuangyexiangmuContent" id="chuangyexiangmuContent-input">
                                   </div>
...
<script>
    ...
    function upload() {
        ...
        $('#chuangyexiangmuPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('chuangyexiangmuPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('chuangyexiangmuPhoto-input').setAttribute('value',photoUrl);
            }
        });


        $('#chuangyexiangmuContentupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                UE.getEditor('chuangyexiangmuContentEditor').execCommand('insertHtml', '<img src=\"' + baseUrl + 'upload/' + data.result.file + '\" width=900 height=560>');
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
           ...
           if($("#jiaoshiId") !=null){
            ...
            httpJson("chuangyexiangmu/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addchuangyexiangmu');
    ...
    function setContent() {
        ...
        <!-- 当前表的填充富文本框 -->
        if (ruleForm.chuangyexiangmuContent != null && ruleForm.chuangyexiangmuContent != 'null' && ruleForm.chuangyexiangmuContent != '' && $("#chuangyexiangmuContentupload").length>0) {

            var chuangyexiangmuContentUeditor = UE.getEditor('chuangyexiangmuContentEditor');
            chuangyexiangmuContentUeditor.ready(function () {
                var mes = '' + ruleForm.chuangyexiangmuContent;
                chuangyexiangmuContentUeditor.setContent(mes);
            });
        }
    }
    // 获取富文本框内容
    function getContent() {
        ...
        <!-- 获取当前表的富文本框内容 -->
        if($("#chuangyexiangmuContentupload").length>0) {
            var chuangyexiangmuContentEditor = UE.getEditor('chuangyexiangmuContentEditor');
            if (chuangyexiangmuContentEditor.hasContents()) {
                $('#chuangyexiangmuContent-input').attr('value', chuangyexiangmuContentEditor.getPlainTxt());
            }
        }
    }
    //数字检查
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                chuangyexiangmuName: "required",
                jiaoshiId: "required",
                chuangyexiangmuPhoto: "required",
                leixTypes: "required",
                chuangyexiangmuContent: "required",
            },
            messages: {
                chuangyexiangmuName: "项目标题不能为空",
                jiaoshiId: "发布教师不能为空",
                chuangyexiangmuPhoto: "样本图片不能为空",
                leixTypes: "项目分类名称不能为空",
                chuangyexiangmuContent: "项目具体内容不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addchuangyexiangmu = window.sessionStorage.getItem("addchuangyexiangmu");
        if (addchuangyexiangmu != null && addchuangyexiangmu != "" && addchuangyexiangmu != "null") {
            window.sessionStorage.removeItem('addchuangyexiangmu');
            //注册表单验证
            $(validform());
            $('#submitBtn').text('新增');
            ...
            window.sessionStorage.removeItem('updateId');
            http("chuangyexiangmu/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
